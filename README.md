...
| `jev_review` | 8 項目 / 5 群の本検査。`fixes` に直す対象、`escalate` に止めどきが返る |
...
現状は `g1_traceability` が unreachable、`g2` と `g4` が fragile。
つまり `block` を出せるのは実質 `structure` と `grounded` の2問だけで、**ゲートは縮退している**。
ルーブリックを HANDOFF 3.5 の構成（18〜22問）に差し替えるまでこの状態が続く。
...
**確認済み**
- MCP の往復と判定ロジックは `npm run check` の 115 項目で通っている（ハンドシェイク、5ツール、
  極性の向き、群FAILの境界、回答欠損で `unknown` になること、s7 が判定に入らないこと、
  `shippable` が存在しないこと、エスカレーション3種の発火と非発火、`unreachable` / `fragile` の
  判別、リポジトリ外パスの拒否）。
...
- 実行環境によっては `ai-gateway.vercel.sh` への egress が塞がれていることがある。
  その場合 probe は 403 で落ちる（鍵の問題ではない）。許可リストに追加するか、
